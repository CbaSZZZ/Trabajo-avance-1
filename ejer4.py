...
"""
Recibe una lista desordenada y crea una lista con un pivote y dos sublistas 
adicionales una con los elementos menores al pivote y la otra con los mayores

Args: Una lista desordenada

Return: Una lista con un pivote y dos sublistas 
adicionales una con los elementos menores al pivote y la otra con los mayores
"""


def particion_quicksort(lista):
  menores = list()
  mayores = list()
  pivote = lista[-1]  
  for x in lista[:-1]:
...
"""
Recibe una lista desordenada y la ordena utilizando la funcion de
particion_quicksort
...
def ordenamiento_lista(lista):
  if len(lista) > 0:
    lista = particion_quicksort(lista)
    cont = 0
    while cont < len(lista):
      ...
      if type(e) == list:
        if len(e) == 0:
          ...
          lista = lista[:cont] + particion_quicksort(e) + lista[cont+1:]
      else:
        cont = cont + 1
    return (lista)
...
particion_quicksort(lista)
nueva_lista = ordenamiento_lista(lista)
if nueva_lista != []:
  cont = 0
  while cont < len(lista_personas):
    for hijos in lista_personas:
      if nueva_lista != []:
        if nueva_lista[-1] == (len(hijos['cantidad_hijos'])):
...
for persona in lista_personas:
    ...
    if any(hijo['genero'] != 'female' for hijo in hijos.values()) and not any(hijo['genero'] == 'female' for hijo in hijos.values()):
        lista_m.append(persona)
    elif any(hijo['genero'] != 'male' for hijo in hijos.values()) and not any(hijo['genero'] == 'male' for hijo in hijos.values()):
        lista_f.append(persona)
    else:
        lista_mix.append(persona)
lista_hijos.append('Personas solo hijos de genero femenino ')
lista_hijos.append(lista_f)
lista_hijos.append('Personas solo hijos de genero masculino ')
lista_hijos.append(lista_m)
lista_hijos.append('Personas con hijos de ambos generos ')
lista_hijos.append(lista_mix)
print(lista_hijos)